...
|-------|-------------|----------|
| `fab` | Container element. It includes a focusable div like <div tabindex="0" role="button" class="btn btn-circle">A</div> and when it gets clicked of focused, it shows more floating buttons | component |
| `fab-close` | Optional class for a close button (or a wrapper of a close button) | part |
| `fab-main-action` | Optional class for the main action button (or a wrapper of the main action button). Either use fab-close or fab-main-action, not both | part |
| `fab-flower` | Modifier class for fab to make it open in a quarter circle arrangement instead of vertical | modifier |
